...
> Ogni processo Q analizza uno spezzone di ogni file di input a cui accede. "n" ed "m" hanno come valore di default rispettivamente 3 e 4 ma possono essere passati come argomenti.
...
> Un processo C ("Counter") principale tiene traccia dei conteggi (ricevendo quindi i dati) i quali devono essere eseguiti da processi separati da esso generati.
...
> Un processo/sottosistema "M" ("Main") principale per la gestione generale: ha solo funzione di "gateway" e interfaccia-utente. 
DEVE richiamare gli altri due processi/sottosistemi di seguito indicati (che devono essere utilizzabili anche autonomamente direttamente da cli).
...
* Per analizzare piu' file in posizione "dir_1, dir_2, ..., r_n" e' sufficiente digitare "./bin/M <dir_1> <dir_2> ... <dir_n>";
* Per settare n ed m basta aggiungere il flag "-setn" o "-setm" a seconda del valore da cambiare, per esempio: "./bin/M <dir> -setn <num> -setm <num>";
...
* Per settare n ed m basta aggiungere il flag "-setn" o "-setm" a seconda del valore da cambiare, per esempio: "./bin/A <dir> -setn <num> -setm <num>";
